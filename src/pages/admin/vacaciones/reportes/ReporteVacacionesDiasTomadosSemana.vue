<template>
  <div class="container mx-auto p-4 pb-8">
    <h1 class="text-3xl font-semibold mb-6 text-indigo-600">Reporte de Días Tomados por Semana</h1>

    <p class="mb-4 text-gray-700">
      Selecciona un año para generar y descargar el reporte de días de vacaciones utilizados por semana.
    </p>

    <div class="mb-6">
      <UiDescriptionList
        title="Información del Reporte"
        description="Este reporte muestra la cantidad de días de vacaciones disfrutados por semana."
        :items="reporteInfo"
      />
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
      ...
      <UiInputText
        ...
        placeholder="reporte_vacaciones_dias_por_semana"
        class="w-full sm:w-1/3"
      />
    </div>
...
const reporteInfo = ref([
  { label: 'Semana', value: 'Se incluirá el número de semana del año seleccionado.' },
  { label: 'Total de Días', value: 'Se incluirá la cantidad total de días de vacaciones disfrutados en esa semana.' },
])
...
const generarReporte = async () => {
  ...
  try {
    const filename = nombreArchivo.value || `reporte_vacaciones_dias_por_semana_${anio.value}`
    const { blob, filename: finalFilename } = await VacacionesService.exportarDiasTomadosPorSemana(
      anio.value,
      formato.value,
      filename
    )
