<template>
  <div class="container mx-auto p-4 pb-8">
    <h1 class="text-3xl font-semibold mb-6 text-indigo-600">Reporte de Vacaciones por Departamento</h1>

    <p class="mb-4 text-gray-700">
      Selecciona un departamento para generar y descargar el reporte de vacaciones correspondientes.
    </p>

    <div class="mb-6">
      <UiDescriptionList
        title="Información del Reporte"
        description="Este reporte incluye todas las vacaciones aprobadas agrupadas por departamento."
        :items="reporteInfo"
      />
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
      ...
      <UiInputText
        ...
        placeholder="reporte_vacaciones_departamento"
        class="w-full sm:w-1/3"
      />
    </div>

    <div class="flex justify-center mt-6">
      <UiButton variant="primary" size="large" :disabled="!departamentoId" @click="generarReporte">
...
// Opciones para select de departamentos
...
const cargarDepartamentos = async () => {
  try {
    ...
    departamentoOptions.value = departamentos.map(d => ({
      label: d.nombre,
      value: d.id
    }))
  } catch (error) {
    console.error('Error al cargar la lista de departamentos:', error)
    useToastService().error('No se pudo cargar la lista de departamentos.', { title: 'Error' })
  }
}
...
const generarReporte = async () => {
  if (!departamentoId.value) {
    useToastService().warning('Por favor, selecciona un departamento.', { title: 'Advertencia' })
    return
  }
  ...
  try {
    const filename = nombreArchivo.value || `reporte_vacaciones_departamento_${departamentoId.value}`
    const { blob, filename: finalFilename } = await VacacionesService.exportarPorDepartamento(
