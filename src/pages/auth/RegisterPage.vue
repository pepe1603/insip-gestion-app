...
<div class="flex flex-col-reverse">
  ...
  <form @submit.prevent="handleRegister" class="mt-3 flex flex-col items-center text-sm sm:text-base">
    ...
    <div class="bg-blue-50 rounded-lg shadow-xs p-2.5 container">
        <p class="font-semibold">Crea tu cuenta con tus propios datos</p>
        <p class="text-gray-600 text-sm">ingresa tu correo electronico de empleado para poder registrarte correctmante</p>
    </div>
...
const handleRegister = async (confirmed = false) => {
  ...
  try {
    const payload = {
      ...
      ...(confirmed ? { confirmed_as_user: true } : {}),
    };
    ...
    router.push('/login'); // NO login automático
